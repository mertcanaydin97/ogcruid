...
require('./cruid_tinymce');
window.cruidTinyMCE = require('./cruid_tinymce_config');
require('./cruid_ace_editor');
...
$(document).ready(function () {
    ...
    $('#cruid-loader').fadeOut();

    $(".hamburger, .navbar-expand-toggle").on('click', function () {
        appContainer.toggleClass("expanded");
        $(this).toggleClass('is-active');
        if ($(this).hasClass('is-active')) {
            window.localStorage.setItem('cruid.stickySidebar', true);
        } else {
            window.localStorage.setItem('cruid.stickySidebar', false);
        }
    });
    ...
    $(document).on('click', '.panel-heading a.panel-action[data-toggle="panel-collapse"]', function (e) {
        ...
        if (!$this.hasClass('panel-collapsed')) {
            ...
            $this.removeClass('cruid-angle-up').addClass('cruid-angle-down');
        } else {
            $this.parents('.panel').find('.panel-body').slideDown();
            $this.removeClass('panel-collapsed');
            $this.removeClass('cruid-angle-down').addClass('cruid-angle-up');
        }
    });
    ...
    $(document).on('click', '.panel-heading a.panel-action[data-toggle="panel-fullscreen"]', function (e) {
        e.preventDefault();
        var $this = $(this);
        if (!$this.hasClass('cruid-resize-full')) {
            $this.removeClass('cruid-resize-small').addClass('cruid-resize-full');
        } else {
            $this.removeClass('cruid-resize-full').addClass('cruid-resize-small');
        }
