    ...
    @if(class_exists($options->model))
        ...
        @if($options->type == 'belongsTo')
            ...
            @if(isset($view) && ($view == 'browse' || $view == 'read'))
                ...
                @else
                    <p>{{ __('cruid::generic.no_results') }}</p>
                @endif
            ...
            @else
                ...
                <select
                    ...
                    data-get-items-route="{{route('cruid.' . $dataType->slug.'.relation')}}"
            ...
            @else
                <select
                    ...
                    @if(isset($options->taggable) && $options->taggable === 'on')
                        data-route="{{ route('cruid.'.\Illuminate\Support\Str::slug($options->table).'.store') }}"
                        data-label="{{$options->label}}"
                        data-error-message="{{__('cruid::bread.error_tagging')}}"
                    @endif
                    @if($row->required == 1) required @endif
                >
