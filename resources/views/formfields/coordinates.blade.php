...
<div id="coordinates-formfield">
    <coordinates
        inline-template
        ref="coordinates"
        api-key="{{ config('cruid.googlemaps.key') }}"
        :points='@json($dataTypeContent->getCoordinates() && count($dataTypeContent->getCoordinates()) ? $dataTypeContent->getCoordinates() : [[ 'lat' => config('cruid.googlemaps.center.lat'), 'lng' => config('cruid.googlemaps.center.lng') ]])'
        :show-autocomplete="{{ $showAutocomplete }}"
        :show-lat-lng="{{ $showLatLng }}"
        :zoom={{ config('cruid.googlemaps.zoom') }}
    >
        <div>
            <div class="form-group">
                <div class="col-md-5" v-if="showAutocomplete">
                    <label class="control-label">{{ __('cruid::generic.find_by_place') }}</label>
                    <input
                        class="form-control"
                        type="text"
                ...
                <div class="col-md-2" v-if="showLatLng">
                    <label class="control-label">{{ __('cruid::generic.latitude') }}</label>
                    <input
                        class="form-control"
                        type="number"
                        step="any"
                ...
                <div class="col-md-2" v-if="showLatLng">
                    <label class="control-label">{{ __('cruid::generic.longitude') }}</label>
                    <input
                        class="form-control"
                        type="number"
                        step="any"
