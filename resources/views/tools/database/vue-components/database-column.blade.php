...
<tr class="newTableRow">
    ...
    <td>
        <select :value="index.type" @change="onIndexTypeChange"
                ...
                class="form-control">
            <option value=""></option>
            <option value="INDEX">{{ __('cruid::database.index') }}</option>
            <option value="UNIQUE">{{ __('cruid::database.unique') }}</option>
            <option value="PRIMARY">{{ __('cruid::database.primary') }}</option>
        </select>
        <small v-if="column.composite" v-once>{{ __('cruid::database.composite_warning') }}</small>
    </td>
    ...
    <td>
        <div class="btn btn-danger delete-row" @click="deleteColumn"><i class="cruid-trash"></i></div>
    </td>
...
@include('cruid::tools.database.vue-components.database-types')
@include('cruid::tools.database.vue-components.database-column-default')
    ...
    Vue.component('database-column', {
        ...
        methods: {
            ...
            onIndexTypeChange(event) {
                if (this.column.name == '') {
                    return toastr.error("{{ __('cruid::database.name_warning') }}");
                }
