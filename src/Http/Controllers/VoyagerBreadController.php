...
use Og\Cruid\Events\BreadUpdated;
use Og\Cruid\Facades\Cruid;

class CruidBreadController extends Controller
{
    public function index()
    {
        $this->authorize('browse_bread');

        $dataTypes = Cruid::model('DataType')->select('id', 'name', 'slug')->get()->keyBy('name')->toArray();
        ...
        return Cruid::view('cruid::tools.bread.index')->with(compact('dataTypes', 'tables'));
    }
    ...
    public function create(Request $request, $table)
    {
        ...
        $dataType = Cruid::model('DataType')->whereName($table)->first();
        ...
        return Cruid::view('cruid::tools.bread.edit-add', $data);
    }

    private function prepopulateBreadInfo($table)
    {
        $displayName = Str::singular(implode(' ', explode('_', Str::title($table))));
        $modelNamespace = config('cruid.models.namespace', app()->getNamespace());
    ...
    public function store(Request $request)
    {
        ...
        try {
            $dataType = Cruid::model('DataType');
            $res = $dataType->updateDataType($request->all(), true);
            $data = $res
                ? $this->alertSuccess(__('cruid::bread.success_created_bread'))
                : $this->alertError(__('cruid::bread.error_creating_bread'));
            if ($res) {
                event(new BreadAdded($dataType, $data));
            }

            return redirect()->route('cruid.bread.index')->with($data);
        } catch (Exception $e) {
            return redirect()->route('cruid.bread.index')->with($this->alertException($e, 'Saving Failed'));
        }
    }
    ...
    public function edit($table)
    {
        ...
        $dataTypeRelationships = Cruid::model('DataRow')->where('data_type_id', '=', $dataType->id)->where('type', '=', 'relationship')->get();
        $scopes = [];
        if ($dataType->model_name != '') {
            $scopes = $this->getModelScopes($dataType->model_name);
        }

        return Cruid::view('cruid::tools.bread.edit-add', compact('dataType', 'fieldOptions', 'isModelTranslatable', 'tables', 'dataTypeRelationships', 'scopes'));
    }
    ...
    public function update(Request $request, $id)
    {
        ...
        /* @var \Og\Cruid\Models\DataType $dataType */
        try {
            $dataType = Cruid::model('DataType')->find($id);
            ...
            $res = $dataType->updateDataType($request->all(), true);
            $data = $res
                ? $this->alertSuccess(__('cruid::bread.success_update_bread', ['datatype' => $dataType->name]))
                : $this->alertError(__('cruid::bread.error_updating_bread'));
            if ($res) {
            ...
            return back()->with($this->alertException($e, __('cruid::generic.update_failed')));
        }
    }
    ...
    public function destroy($id)
    {
        ...
        $res = Cruid::model('DataType')->destroy($id);
        $data = $res
            ? $this->alertSuccess(__('cruid::bread.success_remove_bread', ['datatype' => $dataType->name]))
            : $this->alertError(__('cruid::bread.error_updating_bread'));
        if ($res) {
        ...
        if (!is_null($dataType)) {
            Cruid::model('Permission')->removeFrom($dataType->name);
        }
    ...
    public function addRelationship(Request $request)
    {
        ...
        try {
            ...
            $className = Cruid::modelClass('DataRow');
            ...
            $newRow->order = intval(Cruid::model('DataType')->find($request->data_type_id)->lastRow()->order) + 1;
    ...
    private function getRelationshipField($request)
    {
        ...
        $dataType = Cruid::model('DataType')->find($request->data_type_id);
        ...
        $existingRow = Cruid::model('DataRow')->where('field', '=', $relationshipField)->first();
        $index = 1;
    ...
    {
        Cruid::model('DataRow')->destroy($id);
