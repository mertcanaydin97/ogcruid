...
class CruidController extends Controller
{
    public function index()
    {
        return Cruid::view('cruid::index');
    }
    ...
    public function logout()
    {
        ...
        return redirect()->route('cruid.login');
    }
    ...
    public function upload(Request $request)
    {
        ...
        $dataType = Cruid::model('DataType')->where('slug', '=', $slug)->firstOrFail();
        ...
        while (Storage::disk(config('cruid.storage.disk'))->exists($path.$filename.'.'.$file->getClientOriginalExtension())) {
        ...
        if (in_array($ext, ['jpeg', 'jpg', 'png', 'gif'])) {
            ...
            if (Storage::disk(config('cruid.storage.disk'))->put($fullPath, (string) $image, 'public')) {
                $status = __('cruid::media.success_uploading');
                $fullFilename = $fullPath;
            } else {
                $status = __('cruid::media.error_uploading');
            }
        } else {
            $status = __('cruid::media.uploading_wrong_type');
        }

        // Return URL for TinyMCE
        return Cruid::image($fullFilename);
    }
