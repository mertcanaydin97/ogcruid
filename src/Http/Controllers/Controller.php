...
use Og\Cruid\Events\FileDeleted;
use Og\Cruid\Http\Controllers\ContentTypes\Checkbox;
use Og\Cruid\Http\Controllers\ContentTypes\Coordinates;
use Og\Cruid\Http\Controllers\ContentTypes\File;
use Og\Cruid\Http\Controllers\ContentTypes\Image as ContentImage;
use Og\Cruid\Http\Controllers\ContentTypes\MultipleCheckbox;
use Og\Cruid\Http\Controllers\ContentTypes\MultipleImage;
use Og\Cruid\Http\Controllers\ContentTypes\Password;
use Og\Cruid\Http\Controllers\ContentTypes\Relationship;
use Og\Cruid\Http\Controllers\ContentTypes\SelectMultiple;
use Og\Cruid\Http\Controllers\ContentTypes\Text;
use Og\Cruid\Http\Controllers\ContentTypes\Timestamp;
use Og\Cruid\Traits\AlertsMessages;
use Validator;
...
abstract class Controller extends BaseController
{
    ...
    public function insertUpdateData($request, $slug, $rows, $data)
    {
        ...
        if ($request->session()->has($slug.'_path') || $request->session()->has($slug.'_uuid')) {
            ...
            if ($old_path != $new_path &&
                !Storage::disk(config('cruid.storage.disk'))->exists($new_path) &&
                Storage::disk(config('cruid.storage.disk'))->exists($old_path)
                )
            {
                $request->session()->forget([$slug.'_path', $slug.'_uuid']);
                Storage::disk(config('cruid.storage.disk'))->move($old_path, $new_path);
                Storage::disk(config('cruid.storage.disk'))->deleteDirectory($folder_path);
            }
        }
    ...
    {
        if ($path && Storage::disk(config('cruid.storage.disk'))->exists($path)) {
            Storage::disk(config('cruid.storage.disk'))->delete($path);
