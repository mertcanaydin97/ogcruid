...
use Og\Cruid\Events\MediaFileAdded;
use Og\Cruid\Facades\Cruid;

class CruidMediaController extends Controller
{
    /** @var string */
    private $filesystem;
    ...
    {
        $this->filesystem = config('cruid.storage.disk');
    }
    ...
    public function index()
    {
        ...
        return Cruid::view('cruid::media.index');
    }
    ...
    public function files(Request $request)
    {
        ...
        foreach ($storageItems as $item) {
            if ($item['type'] == 'dir') {
                ...
                if (empty(pathinfo($item['path'], PATHINFO_FILENAME)) && !config('cruid.hidden_files')) {
                    continue;
                }
    ...
    public function new_folder(Request $request)
    {
        ...
        if (Storage::disk($this->filesystem)->exists($new_folder)) {
            $error = __('cruid::media.folder_exists_already');
        } elseif (Storage::disk($this->filesystem)->makeDirectory($new_folder)) {
            $success = true;
        } else {
            $error = __('cruid::media.error_creating_dir');
        }
        ...
        foreach ($request->get('files') as $file) {
            ...
            if ($file['type'] == 'folder') {
                if (!Storage::disk($this->filesystem)->deleteDirectory($file_path)) {
                    $error = __('cruid::media.error_deleting_folder');
                    $success = false;
                }
            } elseif (!Storage::disk($this->filesystem)->delete($file_path)) {
                $error = __('cruid::media.error_deleting_file');
                $success = false;
            }
        }
    ...
    public function rename(Request $request)
    {
        ...
        if (pathinfo($filename)['extension'] !== pathinfo($newFilename)['extension']) {
            $error = __('cruid::media.error_renaming_ext');
        } else {
            ...
            if (!Storage::disk($this->filesystem)->exists("{$location}/{$newFilename}")) {
                if (Storage::disk($this->filesystem)->move("{$location}/{$filename}", "{$location}/{$newFilename}")) {
                    ...
                    $error = __('cruid::media.error_moving');
                }
            } else {
                $error = __('cruid::media.error_may_exist');
            }
        }
    ...
    public function upload(Request $request)
    {
        ...
        try {
            ...
            $allowedMimeTypes = config('cruid.media.allowed_mimetypes', '*');
            if ($allowedMimeTypes != '*' && (is_array($allowedMimeTypes) && !in_array($request->file->getMimeType(), $allowedMimeTypes))) {
                throw new Exception(__('cruid::generic.mimetype_not_allowed'));
            }
            ...
            $success = true;
            $message = __('cruid::media.success_uploaded_file');
    ...
    public function crop(Request $request)
    {
        ...
        try {
            ...
            $success = true;
            $message = __('cruid::media.success_crop_image');
        } catch (Exception $e) {
            $success = false;
            $message = $e->getMessage();
        }
