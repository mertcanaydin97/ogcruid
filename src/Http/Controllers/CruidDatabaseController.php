...
use Og\Cruid\Database\DatabaseUpdater;
use Og\Cruid\Database\Schema\Column;
use Og\Cruid\Database\Schema\Identifier;
use Og\Cruid\Database\Schema\SchemaManager;
use Og\Cruid\Database\Schema\Table;
use Og\Cruid\Database\Types\Type;
use Og\Cruid\Events\TableAdded;
use Og\Cruid\Events\TableDeleted;
use Og\Cruid\Events\TableUpdated;
use Og\Cruid\Facades\Cruid;

class CruidDatabaseController extends Controller
{
    public function index()
    {
        ...
        return Cruid::view('cruid::tools.database.index')->with(compact('dataTypes', 'tables'));
    }
    ...
    public function create()
    {
        ...
        return Cruid::view('cruid::tools.database.edit-add', compact('db'));
    }
    ...
    public function store(Request $request)
    {
        ...
        try {
            ...
            if (isset($request->create_model) && $request->create_model == 'on') {
                ...
                Artisan::call('cruid:make:model', $params);
            ...
            return redirect()
               ...
               ->with($this->alertSuccess(__('cruid::database.success_create_table', ['table' => $table->name])));
        } catch (Exception $e) {
    ...
    public function edit($table)
    {
        ...
        if (!SchemaManager::tableExists($table)) {
            return redirect()
                ->route('cruid.database.index')
                ->with($this->alertError(__('cruid::database.edit_table_not_exist')));
        }
    ...
    protected function prepareDbManager($action, $table = '')
    {
        ...
        if ($action == 'update') {
            ...
            $db->formAction = route('cruid.database.update', $table);
        } else {
            ...
            $db->formAction = route('cruid.database.store');
        }
    ...
    public function show($table)
    {
        ...
        $model_name = Cruid::model('DataType')->where('name', $table)->pluck('model_name')->first();
        if (isset($model_name)) {
    ...
    public function destroy($table)
    {
        ...
        try {
            ...
            return redirect()
                ->route('cruid.database.index')
                ->with($this->alertSuccess(__('cruid::database.success_delete_table', ['table' => $table])));
        } catch (Exception $e) {
