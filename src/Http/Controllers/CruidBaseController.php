...
use Og\Cruid\Database\Schema\SchemaManager;
use Og\Cruid\Events\BreadDataAdded;
use Og\Cruid\Events\BreadDataDeleted;
use Og\Cruid\Events\BreadDataRestored;
use Og\Cruid\Events\BreadDataUpdated;
use Og\Cruid\Events\BreadImagesDeleted;
use Og\Cruid\Facades\Cruid;
use Og\Cruid\Http\Controllers\Traits\BreadRelationshipParser;

class CruidBaseController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        $dataType = Cruid::model('DataType')->where('slug', '=', $slug)->first();
        ...
        if (!empty($dataTypeContent->first())) {
            foreach (Cruid::actions() as $action) {
        ...
        $view = 'cruid::bread.browse';

        if (view()->exists("cruid::$slug.browse")) {
            $view = "cruid::$slug.browse";
        }

        return Cruid::view($view, compact(
            'actions',
            'dataType',
            'dataTypeContent',
            'isModelTranslatable',
    ...
    public function show(Request $request, $id)
    {
        ...
        $view = 'cruid::bread.read';

        if (view()->exists("cruid::$slug.read")) {
            $view = "cruid::$slug.read";
        }

        return Cruid::view($view, compact('dataType', 'dataTypeContent', 'isModelTranslatable', 'isSoftDeleted'));
    }
    ...
    public function edit(Request $request, $id)
    {
        ...
        $view = 'cruid::bread.edit-add';

        if (view()->exists("cruid::$slug.edit-add")) {
            $view = "cruid::$slug.edit-add";
        }

        return Cruid::view($view, compact('dataType', 'dataTypeContent', 'isModelTranslatable'));
    }
    ...
    public function update(Request $request, $id)
    {
        ...
        if (auth()->user()->can('browse', app($dataType->model_name))) {
            $redirect = redirect()->route("cruid.{$dataType->slug}.index");
        } else {
            $redirect = redirect()->back();
        }

        return $redirect->with([
            'message'    => __('cruid::generic.successfully_updated')." {$dataType->getTranslatedAttribute('display_name_singular')}",
            'alert-type' => 'success',
        ]);
    }
    ...
    public function store(Request $request)
    {
        ...
        if (!$request->has('_tagging')) {
            ...
            return $redirect->with([
                'message'    => __('cruid::generic.successfully_added_new')." {$dataType->getTranslatedAttribute('display_name_singular')}",
                'alert-type' => 'success',
            ]);
        } else {
    ...
    public function destroy(Request $request, $id)
    {
        ...
        $data = $affected
            ? [
                'message'    => __('cruid::generic.successfully_deleted')." {$displayName}",
                'alert-type' => 'success',
            ]
            : [
                'message'    => __('cruid::generic.error_deleting')." {$displayName}",
                'alert-type' => 'error',
            ];

        return redirect()->route("cruid.{$dataType->slug}.index")->with($data);
    }
    ...
    public function restore(Request $request, $id)
    {
        ...
        $data = $res
            ? [
                'message'    => __('cruid::generic.successfully_restored')." {$displayName}",
                'alert-type' => 'success',
            ]
            : [
                'message'    => __('cruid::generic.error_restoring')." {$displayName}",
                'alert-type' => 'error',
            ];
    ...
    public function remove_media(Request $request)
    {
        try {
            ...
            if (!isset($data->{$field})) {
                throw new Exception(__('cruid::generic.field_does_not_exist'), 400);
            }
            ...
            if (@json_decode($multi)) {
                ...
                if (is_null(@json_decode($data->{$field}))) {
                    throw new Exception(__('cruid::json.invalid'), 500);
                }
                ...
                if (is_null($key) || $key === false) {
                    throw new Exception(__('cruid::media.file_does_not_exist'), 400);
                }
            ...
            return response()->json([
                'data' => [
                    'status'  => 200,
                    'message' => __('cruid::media.file_removed'),
                ],
            ]);
        } catch (Exception $e) {
            $code = 500;
            $message = __('cruid::generic.internal_error');
    ...
    public function deleteBreadImages($data, $rows, $single_image = null)
    {
        ...
        foreach ($rows as $row) {
            ...
            foreach ($images_to_remove as $image) {
                ...
                if ($image != config('cruid.user.default_avatar') && (is_null($single_image) || $single_image == $image)) {
    ...
    public function order(Request $request)
    {
        ...
        if (empty($dataType->order_column) || empty($dataType->order_display_column)) {
            return redirect()
            ->route("cruid.{$dataType->slug}.index")
            ->with([
                'message'    => __('cruid::bread.ordering_not_set'),
                'alert-type' => 'error',
            ]);
        }
        ...
        $dataRow = Cruid::model('DataRow')->whereDataTypeId($dataType->id)->whereField($display_column)->first();

        $view = 'cruid::bread.order';

        if (view()->exists("cruid::$slug.order")) {
            $view = "cruid::$slug.order";
        }
