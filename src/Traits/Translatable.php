...
trait Translatable
{
    ...
    {
        return $this->hasMany(Cruid::model('Translation'), 'foreign_key', $this->getKeyName())
            ->where('table_name', $this->getTable())
            ->whereIn('locale', config('cruid.multilingual.locales', []));
    }
    ...
    public function getTranslationsOf($attribute, array $languages = null, $fallback = true)
    {
        if (is_null($languages)) {
            $languages = config('cruid.multilingual.locales', [config('cruid.multilingual.default')]);
        }
    ...
    public function getTranslatedAttributeMeta($attribute, $locale = null, $fallback = true)
    {
        ...
        if (!in_array($attribute, $this->getTranslatableAttributes())) {
            return [$this->getAttribute($attribute), config('cruid.multilingual.default'), false];
        }
        ...
        $default = config('cruid.multilingual.default');
    ...
    public function setAttributeTranslations($attribute, array $translations, $save = false)
    {
        ...
        $default = config('cruid.multilingual.default', 'en');
        $locales = config('cruid.multilingual.locales', [$default]);
    ...
    public function prepareTranslations($request)
    {
        ...
        foreach ($fields as $field) {
            ...
            $request->merge([$field => $trans[config('cruid.multilingual.default', 'en')]]);
    ...
    public function prepareTranslationsFromArray($field, &$requestData)
    {
        ...
        // Set the default local value
        $requestData['display_name'] = $trans[config('cruid.multilingual.default', 'en')];
