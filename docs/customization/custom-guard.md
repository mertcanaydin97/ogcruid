...
Starting with Cruid 1.2 you can define a \(custom\) guard which is used throughout Cruid.  
To do so, just bind the name of your auth-guard to `CruidGuard`.  
...
and a model which extends Cruids user-model:
...
class Admin extends \Og\Cruid\Models\User
{
...
Next you have to tell Cruid to use your new guard.  
